...
\textbf{Check and mention uninstrumented bars? Gap size? Etc. Etc. A real life figure?}
...
Provide plots of diagnostic information, hit data etc etc.
...
Show of all hits in the data files.
...
Same way as simulate hits are used. Build possible tracks. Separate hits by hit time.
...
Using pattern recognition to identify showers (non-muons) from muons (non-showers).
...
\begin{figure}[h!]
\centering
\includegraphics[width=\textwidth]{figures/oldStudies/newChargeZoom5.pdf}

\includegraphics[width=\textwidth]{figures/oldStudies/newChargeZoom1.pdf}
\caption{Initial charge id results}
\label{fig:ChargeInitial}
\end{figure}

These plots confirm that the full data analysis chain works however the results are slightly less than expected from simulations. Looking on an event by event basis it become clear that the events are a mix of muons and pions even after an initial pattern recognition. This motivated the development of a more advanced pattern recognition algorithm using machine learning.
...
Developed to not be dependent on the momentum of the particle, should work for full range.

Essentially the machine learning algorithm identifies muons in a mixed background, but does not identify any particles in the background. This could be a future study and could be used to identify pions as well. Refer to above plot for track and shower, perhaps show that the difficulty is at low pt since then track is shorter.

Signal is always defined as a muon, background as the remaining particles in the beam such as pions etc.

Variables, described in some presentation?
...
Hits per plane
Planes in track
Average hits per used planes
Length of track (Being plotted without modulus)
Not now using deposited energy, do not have it for test beam! Using Muons as data, Pions as background
...
from this chose, MLPBNN Recommended ANN with BFGS training method and bayesian regulator. Could have chosen others but this was the best
...
\caption{Responce with and without overtraining check}
